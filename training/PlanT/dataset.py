...
class PlanTDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if not self.data_cache is None and labels[0] in self.data_cache:
            ...
            for sample_key, file in zip(
                ...
                for labels_data_i in labels_data:
                    ...
                    p = np.append(p,[1]) # array([-3.43537962e+00, 2.80015999e+01, -1.27086047e-02,  1.00000000e+00])
                    p[1] = -p[1] # array([-3.43537962e+00, -2.80015999e+01, -1.27086047e-02,  1.00000000e+00])
                    p_global = ego_matrix @ p # (4,4)
                ...
                if sample_key == "output":
                    ...
                    if self.cfg.model.pre_training.quantize:
                        data_car_no_quantize = data_car # 这样的话没有参与清洗，与上一帧对应不起来
                        if len(data_car) > 0:
                    ...
                    if self.cfg.model.pre_training.pretraining == "forecast":
                        ...
                        data_car_by_id_noq = {}
                        i = 0
                        for ii, x in enumerate(labels_data):# 这里不是用box使用labels_data
                            if x["class"] == "Car" and ((self.cfg_train.remove_back and float(pos[ii][0]) >= 0) or not self.cfg_train.remove_back):
                                data_car_by_id[x["id"]] = data_car[i] # 顺序是不变的
                                data_car_by_id_noq[x["id"]] = data_car_no_quantize[i] # 顺序是不变的
                                i += 1
                                
                        data_car_matched = []
                        data_car_matched_noq = []
                        for i, x in enumerate(data_car_input):
                            input_id = x[7]
                            if input_id in data_car_by_id:
                                data_car_matched.append(data_car_by_id[input_id])
                                data_car_matched_noq.append(data_car_by_id_noq[input_id])
                            else:
                                # append dummy data
                                dummy_data = x
                                dummy_data[0] = 10.0  # type indicator for dummy
                                data_car_matched.append(dummy_data)
                                data_car_matched_noq.append(dummy_data)

                        data_car = data_car_matched
                        data_car_no_quantize = data_car_matched_noq
                        assert len(data_car) == len(data_car_input)
                        assert len(data_car) == len(data_car_no_quantize)

                else:
                    data_car_no_quantize = data_car
                    data_car_input = data_car

                # remove id from data_car
                data_car_idx = [x[-1] for x in data_car_no_quantize]
                sample[f'{sample_key}_idx'] = np.array(data_car_idx) # output的idx也非常关心，这里可以保证是准确的，这些idx在两帧是一样的，忽略新出现的东西
                # 这些都是不包含route的
                
                if sample_key == 'output':
                    data_car_idx = [x[0]==10 for x in data_car_no_quantize]
                    sample[f'{sample_key}_disappear'] = np.array(data_car_idx)
                ...
                sample[f"{sample_key}_label_path"] = label_path
                # 下面concat上route有几个的信息
                sample[f'{sample_key}_idx'] = np.concatenate([sample[f'{sample_key}_idx'], np.array(data_route)[:,4]]) # route_id
                # 这里呢concat上了route的id部分
        ...
        self.cnt+=1
        # import pdb;pdb.set_trace()
        assert len(sample['input_idx']) == len(sample['output_idx']) # TODO
        return sample # 这里只是返回一个字典
...
def generate_batch(data_batch):
    ...
    input_idx_batch = []
    output_idx_batch = []
    output_disappear_batch = []
    output_label_path_batch = []
    for element_id, sample in enumerate(data_batch):
        ...
        input_indices = torch.tensor([element_id] * len(input_item)).unsqueeze(1) # 表示这18个东西是属于batch_id=0
        ...
        input_idx = torch.tensor(sample["input_idx"])
        output_idx = torch.tensor(sample["output_idx"])
        output_disappear = torch.tensor(sample["output_disappear"])
        # output_label_path = sample["output_label_path"]
        output_label_path = sample["input_label_path"] # 更合理一些
        # 在fn这里进行错位，在getitem的时候是对的
        
        input_idx_batch.append(input_idx)
        output_idx_batch.append(output_idx)
        output_disappear_batch.append(output_disappear)
        output_label_path_batch.append(output_label_path)
        # input_indices表示的是当前在哪个batch
    ...
    if has_idx: # 仿真结束之后提取标签
        return input_batch, output_batch, waypoints_batch, tp_batch, light_batch, input_idx_batch, output_idx_batch, output_disappear_batch, output_label_path_batch
    else: # 仿真的时候
