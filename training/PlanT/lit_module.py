...
class LitHFLM(pl.LightningModule):
    ...
    def forward(self, x, y=None, target_point=None, light_hazard=None, input_idx_batch=None, output_idx_batch=None, output_disappear_batch=None, output_label_path_batch=None):
        return self.model(x, y, target_point, light_hazard, input_idx_batch, output_idx_batch, output_disappear_batch, output_label_path_batch)
    ...
    def training_step(self, batch, batch_idx):
        # 进入train_step的时候，每个输入、输出已经被编码好了
        x, y, wp, tp, light, input_idx_batch, output_idx_batch, output_disappear_batch, output_label_path_batch = batch

        # training with only waypoint loss
        if self.cfg.model.pre_training.get("pretraining", "none") == "none":
            logits, pred_wp, _ = self(x, y, tp, light, input_idx_batch, output_idx_batch, output_disappear_batch, output_label_path_batch)
            ...
            if self.cfg.model.pre_training.get("multitask", False) == True:
                # multitask training
                logits, targets, pred_wp, _ = self(x, y, tp, light, input_idx_batch, output_idx_batch, output_disappear_batch, output_label_path_batch)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if self.cfg.model.pre_training.get("pretraining", "none") == "none":
            ...
            if self.cfg.model.pre_training.get("multitask", False) == True:
                # multitask training
                logits, targets, pred_wp, _ = self(x, y, tp, light, input_idx_batch, output_idx_batch, output_disappear_batch, output_label_path_batch) ## x,y分别是输入和gt输出
                loss_wp = F.l1_loss(pred_wp, wp)
                losses_forecast = [
                    torch.mean(self.criterion_forecast(logits[i], targets[i].squeeze()))
                    for i in range(len(logits))
