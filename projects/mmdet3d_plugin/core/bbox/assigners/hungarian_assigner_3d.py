...
class HungarianAssigner3D(BaseAssigner):
    ...
    def assign(self,
        ...
        assigned_labels = bbox_pred.new_full((num_bboxes, ),
                                             -1,
                                             dtype=torch.long)
        new_gt_idxs = bbox_pred.new_full((num_bboxes, ),
                                             -1,
                                             dtype=torch.float64)
        ...
        cls_cost = self.cls_cost(cls_pred, gt_labels) # torch.Size([50, 3])
        # regression L1 cost
        normalized_gt_bboxes = normalize_bbox(gt_bboxes, self.pc_range)
        reg_cost = self.reg_cost(bbox_pred[:, :8], normalized_gt_bboxes[:, :8])
      
        # weighted sum of above two costs
        cost = cls_cost + reg_cost # torch.Size([50, 3])
        
        if torch.isnan(cost).any() or torch.isinf(cost).any():
            print('cost has nan or inf values')
        ...
        matched_col_inds = torch.from_numpy(matched_col_inds).to(
            bbox_pred.device)
        gt_idxs = torch.from_numpy(gt_idxs).to(
            bbox_pred.device)
