...
class LeaderboardEvaluator(object):
    ...
    def __init__(self, args, statistics_manager):
        ...
        """
        print(f"\nLeaderboardEvaluator start init...")
        sys.stdout.flush()
        ...
        print(args.host, int(args.port))
        sys.stdout.flush()
        self.client = carla.Client(args.host, int(args.port))
        if args.timeout:
            self.client_timeout = float(args.timeout) # 默认是600s恐怖
        self.client.set_timeout(self.client_timeout)
        print(f"self.client built...")
        sys.stdout.flush()
        ...
        print(f"self.traffic_manager built...")
        sys.stdout.flush()
        ...
        # Load agent
        module_name = str(args.agent.replace('/','.')[:-3]) # carla_agent_files.PlanTPerception_submissionagent
        ...
        print(f"LeaderboardEvaluator built...")
        sys.stdout.flush()
    ...
    def _load_and_run_scenario(self, args, config):
        ...
        try:
            ...
            if int(os.environ['DATAGEN'])==1:
                self.agent_instance = getattr(self.module_agent, agent_class_name)(args.agent_config, config.index, args, world=self.world, spectator=self.spectator) # CHANGED        
            else:
                self.agent_instance = getattr(self.module_agent, agent_class_name)(args.agent_config, route_date_string, args, world=self.world, spectator=self.spectator) # CHANGED
