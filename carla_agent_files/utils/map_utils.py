...
def encode_npy_to_pil(bev_array):#(15, 500, 500)dtype('float32')
    """
    ...
    bev = np.ceil(bev_array).astype('uint8')#(15, 500, 500)
    ...
    for i in range(channels):
        if i<=1 and i >= 0:
            ...
            if i==2:
                ...
                color = [0,255,0]
            img[:,x,y] = np.array(color).reshape(-1,1)
        elif i<=9 and i >= 5:
            # Vehicle, Pedestrian
