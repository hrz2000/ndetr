...
class Renderer():
    def __init__(self, map_offset, map_dims, data_generation=True):
        ...
        if data_generation:
            ...
            hw = 300
            self.local_view_dims = (hw, hw)
            self.crop_dims = (hw, hw)
        else:
    ...
    def render_agent_bv(
            self, 
            grid, 
            grid_pos, #tensor([-41.4886, 112.9453], device='cuda:0')
            grid_orientation, #tensor([-1.5736], device='cuda:0')
            vehicle, 
            position, #tensor([-56.8447, 137.0356], device='cuda:0')
            orientation,#yaw弧度tensor([0.0061], device='cuda:0')
            channel=5,
        ...
        pos_pix_bv = self.world_to_pix_crop(position, grid_pos, grid_orientation)#tensor([172.8813, 370.2350], device='cuda:0')
