...
# Define a function to save the image every 10 frames
def save_image(image):
    ...
    if frame_number % 10 == 0:
        ...
        mmcv.imwrite(np.array(image), f'{save_path}/spectator/{frame_number%10:05d}.png')
...
class AutoPilot(autonomous_agent_local.AutonomousAgent):
    def setup(self, path_to_conf_file, route_index=None, cfg=None, exec_or_inter=None):
        ...
        if isinstance(self.route_index, int):
            self.route_index = f"route_{self.route_index}"
        ...
        # (self.save_path / 'spectator').mkdir()
        # global save_path
    ...
    def _init(self, hd_map):
        ...
        # # Create a location object with x, y, and z coordinates
        # location = carla.Location(x=0, y=0, z=0)
        # # Create a rotation object with pitch, yaw, and roll angles
        # rotation = carla.Rotation(pitch=0, yaw=0, roll=0) # Use different angles to change the direction
        # # Create a transform object with location and rotation
        # transform = carla.Transform(location, rotation)
        
        # self.camera = self._world.spawn_actor(camera_bp, transform, attach_to=spectator)
    ...
    def run_step(self, input_data, timestamp, keep_ids=None, sensors=None):
        ...
        control = self._get_control_autopilot(input_data)
        return control


    def _get_control_autopilot(self, input_data, steer=None, throttle=None,
        ...
        # input_data['target_point'] = target_point # array([26.37936405, -2.02509578])
        ...
        if((self.step % self.save_freq == 0) and (self.save_path is not None)) or self.all_time==True:
            my_labels = self.save(target_point, next_target_point, steer, throttle, brake, target_speed, tick_data)
            input_data['my_labels'] = my_labels
        return control
